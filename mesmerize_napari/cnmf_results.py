...
def show_results(cnmf_obj, viewer):
    ...
    contours_good = caiman_get_contours(
        cnmf_obj.estimates.A[:, cnmf_obj.estimates.idx_components], dims, swap_dim=True
    )
    ...
    if (
        cnmf_obj.estimates.idx_components_bad is not None
        and len(cnmf_obj.estimates.idx_components_bad) > 0
    ):
